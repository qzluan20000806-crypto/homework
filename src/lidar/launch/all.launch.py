...
def generate_launch_description():
    ...
    lidar_node = LoadComposableNodes(
        target_container='lidar',
        composable_node_descriptions=[
            ComposableNode(
                package='lidar',
                plugin='RosPointCloudBagPublisher', 
    ...
    dbscan_node = LoadComposableNodes(
        target_container='lidar',
        composable_node_descriptions=[
            ComposableNode(
                package='lidar',
                plugin='DBSCAN_component', 
                name='DBSCAN_component',
                extra_arguments=[{'use_intra_process_comms': True}]  # 进程内通信（零拷贝优化）
            )
        ]
    )
    ...
    # ROS configuration
    pointcloud_topic = LaunchConfiguration("cloud_topic", default="/cloud_topic")
